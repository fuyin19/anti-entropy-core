...
`ref/` is a prepared, read-only projection of the outer human-owned references. `.agent-workbench.json` describes the projection; every other declared item root is a Knowledge Unit Envelope v2. Cross-check warning-bearing conversions against another representation or retained source when accuracy matters.
